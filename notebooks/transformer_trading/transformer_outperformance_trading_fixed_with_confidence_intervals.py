...
def add_features(frame: pd.DataFrame, forecast_horizon_days: int) -> tuple[pd.DataFrame, list[str]]:
    ...
    df["target_outperform"] = (
        df["future_stock_log_return"] > df["future_benchmark_log_return"]
    ).astype(float)
...
def run_pipeline(cfg: Config) -> dict:
    ...
    feature_frame, feature_columns = add_features(raw_prices, cfg.forecast_horizon_days)
    ...
    latest = test_predictions.iloc[-1]
