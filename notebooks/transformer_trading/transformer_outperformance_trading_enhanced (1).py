#!/usr/bin/env python3
"""
Enhanced Transformer outperformance trading script.

What this script predicts:
    1. Three-class relative regime over the next N trading days:
       bearish / neutral / bullish for TICKER relative to BENCHMARK.
    2. Future excess log return:
       future_stock_log_return - future_benchmark_log_return.

Important:
    The model is not a direct future-price forecaster.
    Approximate future prices are derived from historical conditional excess-return
    distributions plus the model's probability of outperformance.
"""
...
class Config:
    ...
    transformer_walk_forward_epochs: int = 8
    transaction_cost_bps: float = 5.0
    allow_short: bool = True
    neutral_zone_bps: float = 75.0
    focal_gamma: float = 2.0
    classification_loss_weight: float = 1.0
    regression_loss_weight: float = 0.35
    gradient_clip_norm: float = 1.0
    scheduler_patience: int = 2
    scheduler_factor: float = 0.5
    include_vix: bool = True
    regime_symbols: tuple[str, ...] = field(default_factory=lambda: ("QQQ", "SMH", "SOXX", "XLK"))
...
def download_ohlcv(
    ...
    extras = list(regime_symbols or [])
    requested_symbols: list[str] = []
    for symbol in [ticker.upper(), benchmark.upper(), *extras]:
        if symbol not in requested_symbols:
            requested_symbols.append(symbol)
    if include_vix and "^VIX" not in requested_symbols:
        requested_symbols.append("^VIX")
    ...
    stock = extract_symbol_frame(ticker.upper())[["open", "high", "low", "close", "volume"]]
    benchmark_frame = extract_symbol_frame(benchmark.upper()).rename(columns=lambda name: f"benchmark_{name}")
    combined = stock.join(benchmark_frame[["benchmark_close"]], how="inner")

    for symbol in extras:
        normalized = safe_symbol_name(symbol)
        extra_frame = extract_symbol_frame(symbol.upper())
        if "close" in extra_frame:
            combined = combined.join(extra_frame[["close"]].rename(columns={"close": f"{normalized}_close"}), how="left")

    if include_vix:
        try:
            vix_frame = extract_symbol_frame("^VIX")
            if "close" in vix_frame:
                combined = combined.join(vix_frame[["close"]].rename(columns={"close": "vix_close"}), how="left")
        except Exception:
            pass

    combined = combined.sort_index().replace([np.inf, -np.inf], np.nan)
    combined = combined.dropna(subset=["open", "high", "low", "close", "volume", "benchmark_close"])
    if combined.empty:
        raise ValueError("No overlapping stock and benchmark history was found.")
    return combined


def rolling_beta(asset_returns: pd.Series, benchmark_returns: pd.Series, window: int = 20) -> pd.Series:
...
def drawdown_series(prices: pd.Series, window: int = 20) -> pd.Series:
    rolling_max = prices.rolling(window).max()
    return prices / rolling_max - 1.0


def class_from_excess_return(excess_return: pd.Series, neutral_zone: float) -> pd.Series:
    classes = np.full(len(excess_return), CLASS_NEUTRAL, dtype=np.int64)
    values = excess_return.to_numpy(dtype=float)
    classes[values > neutral_zone] = CLASS_BULLISH
    classes[values < -neutral_zone] = CLASS_BEARISH
    return pd.Series(classes, index=excess_return.index, dtype="int64")


def add_features(
    frame: pd.DataFrame,
    forecast_horizon_days: int,
    neutral_zone_bps: float,
    ...
    df["benchmark_log_return_1d"] = np.log(df["benchmark_close"] / df["benchmark_close"].shift(1))
    df["benchmark_log_return_5d"] = np.log(df["benchmark_close"] / df["benchmark_close"].shift(5))
    df["benchmark_log_return_20d"] = np.log(df["benchmark_close"] / df["benchmark_close"].shift(20))

    df["volatility_10d"] = df["log_return_1d"].rolling(10).std()
    df["volatility_20d"] = df["log_return_1d"].rolling(20).std()
    df["benchmark_volatility_20d"] = df["benchmark_log_return_1d"].rolling(20).std()
    df["rolling_beta_20d"] = rolling_beta(df["log_return_1d"], df["benchmark_log_return_1d"], window=20)

    df["range_pct"] = (df["high"] - df["low"]) / df["close"]
    df["benchmark_drawdown_20d"] = drawdown_series(df["benchmark_close"], window=20)
    df["benchmark_rel_5d"] = df["log_return_5d"] - df["benchmark_log_return_5d"]
    df["benchmark_rel_20d"] = df["log_return_20d"] - df["benchmark_log_return_20d"]
    ...
    for column in sorted(col for col in df.columns if col.endswith("_close") and col not in {"benchmark_close"}):
        prefix = column[:-6]
        if prefix == "vix":
            df["vix_level"] = df[column]
            df["vix_log_return_5d"] = np.log(df[column] / df[column].shift(5))
            df["vix_zscore_20d"] = (df[column] - df[column].rolling(20).mean()) / df[column].rolling(20).std()
            extra_feature_columns.extend(["vix_level", "vix_log_return_5d", "vix_zscore_20d"])
            continue
        df[f"{prefix}_log_return_5d"] = np.log(df[column] / df[column].shift(5))
        df[f"rel_strength_{prefix}_5d"] = df["log_return_5d"] - df[f"{prefix}_log_return_5d"]
        df[f"benchmark_rel_strength_{prefix}_5d"] = df["benchmark_log_return_5d"] - df[f"{prefix}_log_return_5d"]
        extra_feature_columns.extend(
            [
                f"{prefix}_log_return_5d",
                f"rel_strength_{prefix}_5d",
                f"benchmark_rel_strength_{prefix}_5d",
            ]
        )
    ...
    df["future_excess_log_return"] = df["future_stock_log_return"] - df["future_benchmark_log_return"]

    neutral_zone = neutral_zone_bps / 10000.0
    df["target_outperform"] = (df["future_excess_log_return"] > 0.0).astype(float)
    df["target_class"] = class_from_excess_return(df["future_excess_log_return"], neutral_zone)
    ...
    if require_future_returns:
        required.extend(["future_stock_log_return", "future_benchmark_log_return", "future_excess_log_return"])
    ...
    if require_future_returns:
        ...
        known_future = df["future_excess_log_return"].notna()
        df["target_outperform"] = np.where(known_future, (df["future_excess_log_return"] > 0.0).astype(float), np.nan)
        df["target_class"] = np.where(
            known_future,
            class_from_excess_return(df["future_excess_log_return"], neutral_zone),
            np.nan,
        )
...
class SequenceDataset(Dataset):
    def __init__(self, features: np.ndarray, class_targets: np.ndarray, excess_targets: np.ndarray):
        self.features = torch.tensor(features, dtype=torch.float32)
        self.class_targets = torch.tensor(class_targets, dtype=torch.long)
        self.excess_targets = torch.tensor(excess_targets, dtype=torch.float32)

    def __len__(self) -> int:
        return len(self.features)

    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return self.features[index], self.class_targets[index], self.excess_targets[index]
...
def build_sequence_bundle(frame: pd.DataFrame, feature_columns: Iterable[str], sequence_length: int) -> SequenceBundle:
    ...
    binary_targets = frame["target_outperform"].to_numpy(dtype=np.float32)
    class_targets = frame["target_class"].to_numpy(dtype=np.int64)
    excess_targets = frame["future_excess_log_return"].to_numpy(dtype=np.float32)
    ...
    sequence_features = []
    sequence_binary_targets = []
    sequence_class_targets = []
    sequence_excess_targets = []
    sequence_dates = []
    sequence_close = []
    ...
    for end_index in range(sequence_length, len(frame)):
        ...
        sequence_binary_targets.append(binary_targets[end_index])
        sequence_class_targets.append(class_targets[end_index])
        sequence_excess_targets.append(excess_targets[end_index])
    ...
    return SequenceBundle(
        ...
        targets=np.asarray(sequence_binary_targets, dtype=np.float32),
        class_targets=np.asarray(sequence_class_targets, dtype=np.int64),
        excess_targets=np.asarray(sequence_excess_targets, dtype=np.float32),
...
def slice_bundle(bundle: SequenceBundle, subset_slice: slice) -> SequenceBundle:
    return SequenceBundle(
        features=bundle.features[subset_slice],
        targets=bundle.targets[subset_slice],
        class_targets=bundle.class_targets[subset_slice],
        excess_targets=bundle.excess_targets[subset_slice],
...
def partition_from_bundle(bundle: SequenceBundle, features: np.ndarray) -> dict[str, np.ndarray]:
    return {
        "features": features,
        "targets": bundle.targets,
        "class_targets": bundle.class_targets,
        "excess_targets": bundle.excess_targets,
        "dates": bundle.dates,
...
def split_bundle(bundle: SequenceBundle, train_ratio: float, valid_ratio: float) -> tuple[dict[str, dict[str, np.ndarray]], StandardScaler]:
    ...
    scaler.fit(bundle.features[train_slice].reshape(-1, bundle.features.shape[-1]))

    partitions = {}
    for name, subset_slice in {"train": train_slice, "valid": valid_slice, "test": test_slice}.items():
        subset_bundle = slice_bundle(bundle, subset_slice)
        partitions[name] = partition_from_bundle(subset_bundle, scale_sequence_features(subset_bundle.features, scaler))
...
def make_fold_partitions(
    ...
    valid_size = max(20, int(train_end * valid_ratio))
    valid_size = min(valid_size, max(1, train_end // 3))
    train_core_end = train_end - valid_size
    if train_core_end < 32:
        raise ValueError("Not enough history for train/validation split inside fold.")

    train_bundle = slice_bundle(bundle, slice(0, train_core_end))
    valid_bundle = slice_bundle(bundle, slice(train_core_end, train_end))
    test_bundle = slice_bundle(bundle, slice(test_start, test_end))

    scaler = StandardScaler()
    scaler.fit(train_bundle.features.reshape(-1, train_bundle.features.shape[-1]))
    partitions = {
        "train": partition_from_bundle(train_bundle, scale_sequence_features(train_bundle.features, scaler)),
        "valid": partition_from_bundle(valid_bundle, scale_sequence_features(valid_bundle.features, scaler)),
        "test": partition_from_bundle(test_bundle, scale_sequence_features(test_bundle.features, scaler)),
    }
    return partitions, scaler
...
def class_probabilities_to_binary_probability(class_probabilities: np.ndarray) -> np.ndarray:
    class_probabilities = np.asarray(class_probabilities, dtype=float)
    return class_probabilities[:, CLASS_BULLISH] + 0.5 * class_probabilities[:, CLASS_NEUTRAL]


def compute_class_weights(class_targets: np.ndarray) -> torch.Tensor:
    counts = np.bincount(class_targets.astype(int), minlength=NUM_CLASSES).astype(float)
...
def multiclass_focal_loss(logits: torch.Tensor, targets: torch.Tensor, class_weights: torch.Tensor, gamma: float) -> torch.Tensor:
    ce = F.cross_entropy(logits, targets, weight=class_weights, reduction="none")
    pt = torch.softmax(logits, dim=-1).gather(1, targets.unsqueeze(1)).squeeze(1).clamp(1e-6, 1.0)
    return ((1.0 - pt) ** gamma * ce).mean()


def build_model(cfg: Config, num_features: int) -> TransformerMultiTaskModel:
...
def make_loaders(partitions: dict[str, dict[str, np.ndarray]], batch_size: int) -> dict[str, DataLoader]:
    loaders = {}
    for name, partition in partitions.items():
        dataset = SequenceDataset(partition["features"], partition["class_targets"], partition["excess_targets"])
        loaders[name] = DataLoader(dataset, batch_size=batch_size, shuffle=(name == "train"))
    return loaders


def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, class_weights: torch.Tensor, cfg: Config) -> float:
    model.train()
    losses = []
    for features, class_targets, excess_targets in loader:
        features = features.to(DEVICE)
        class_targets = class_targets.to(DEVICE)
        excess_targets = excess_targets.to(DEVICE)

        optimizer.zero_grad()
        class_logits, excess_prediction = model(features)
        classification_loss = multiclass_focal_loss(class_logits, class_targets, class_weights, gamma=cfg.focal_gamma)
        regression_loss = F.smooth_l1_loss(excess_prediction, excess_targets)
        loss = cfg.classification_loss_weight * classification_loss + cfg.regression_loss_weight * regression_loss
        loss.backward()
...
def safe_auc(y_true: np.ndarray, prob: np.ndarray) -> float:
    try:
        return float(roc_auc_score(y_true.astype(int), prob.astype(float)))
    except ValueError:
        return float("nan")


@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, class_weights: torch.Tensor, cfg: Config) -> dict[str, Any]:
    model.eval()
    losses = []
    class_probabilities_all = []
    class_targets_all = []
    excess_predictions_all = []
    excess_targets_all = []
    ...
    for features, class_targets, excess_targets in loader:
        ...
        losses.append(float(loss.item()))
        class_probabilities_all.append(torch.softmax(class_logits, dim=-1).cpu().numpy())
        class_targets_all.append(class_targets.cpu().numpy())
        excess_predictions_all.append(excess_prediction.cpu().numpy())
        excess_targets_all.append(excess_targets.cpu().numpy())

    class_probabilities_np = np.concatenate(class_probabilities_all)
    class_targets_np = np.concatenate(class_targets_all).astype(int)
    excess_predictions_np = np.concatenate(excess_predictions_all).astype(float)
    excess_targets_np = np.concatenate(excess_targets_all).astype(float)
    binary_targets_np = (class_targets_np == CLASS_BULLISH).astype(int)
    probability_outperform = class_probabilities_to_binary_probability(class_probabilities_np)
    class_predictions = class_probabilities_np.argmax(axis=1)

    return {
        "loss": float(np.mean(losses)),
        "auc": safe_auc(binary_targets_np, probability_outperform),
        "accuracy": float(accuracy_score(binary_targets_np, probability_outperform >= 0.5)),
        "multiclass_accuracy": float(accuracy_score(class_targets_np, class_predictions)),
        "excess_mae": float(np.mean(np.abs(excess_predictions_np - excess_targets_np))),
        "excess_rmse": float(np.sqrt(np.mean((excess_predictions_np - excess_targets_np) ** 2))),
        "class_probabilities": class_probabilities_np,
        "class_targets": class_targets_np,
        "class_predictions": class_predictions,
        "probabilities": probability_outperform,
        "targets": binary_targets_np,
        "expected_excess_log_return": excess_predictions_np,
        "excess_targets": excess_targets_np,
    }


def logits_from_probability(probability: np.ndarray) -> np.ndarray:
...
def fit_platt_calibrator(valid_logits: np.ndarray, valid_targets: np.ndarray, enabled: bool = True) -> ProbabilityCalibrator:
    ...
    return ProbabilityCalibrator(enabled=True, intercept=float(model.intercept_[0]), coef=float(model.coef_[0][0]))
...
def lightgbm_baseline_is_safe() -> bool:
    return LGBMClassifier is not None and platform.system() != "Darwin"


def make_baseline_models(seed: int = SEED) -> dict[str, Any]:
    models: dict[str, Any] = {
    ...
    for model_name, model in baseline_models.items():
        try:
            ...
            for split in ["valid", "test"]:
                ...
                rows.append({"model": model_name, "split": split, **classification_metrics(y, prob)})
        except Exception as exc:
            rows.append({"model": model_name, "split": "error", "accuracy": np.nan, "auc": np.nan, "brier": np.nan, "error": str(exc)})
    if platform.system() == "Darwin" and LGBMClassifier is not None and "lightgbm" not in baseline_models:
        rows.append({"model": "lightgbm", "split": "skipped", "error": "Skipped on Darwin because LightGBM segfaults in this environment."})
...
def run_walk_forward_validation(bundle: SequenceBundle, cfg: Config, output_dir: Path) -> pd.DataFrame:
    ...
    for fold in range(folds):
        ...
        x_train = flatten_features(scale_sequence_features(train_raw, scaler))
        x_test = flatten_features(scale_sequence_features(test_raw, scaler))
    ...
    if not df.empty and "auc" in df.columns:
        df.groupby("model", dropna=False)[["accuracy", "auc", "brier"]].agg(["mean", "std", "count"]).to_csv(
...
def backtest_position_from_row(row: pd.Series, bullish_threshold: float, bearish_threshold: float, minimum_edge_bps: float, allow_short: bool) -> int:
    probability = float(row["probability_outperform"])
    expected_edge = float(row.get("expected_excess_log_return", 0.0))
    minimum_edge = minimum_edge_bps / 10000.0
    if probability >= bullish_threshold and expected_edge >= minimum_edge:
        return 1
    if allow_short and probability <= bearish_threshold and expected_edge <= -minimum_edge:
        return -1
    return 0
...
def run_backtest(
    ...
    trades = []
    previous_position = 0
    use_bullish = cfg.bullish_threshold if bullish_threshold is None else bullish_threshold
    use_bearish = cfg.bearish_threshold if bearish_threshold is None else bearish_threshold
    use_edge_bps = cfg.minimum_edge_bps if minimum_edge_bps is None else minimum_edge_bps

    for i in range(0, len(df), step):
        row = df.iloc[i]
        position = backtest_position_from_row(row, use_bullish, use_bearish, use_edge_bps, cfg.allow_short)
        ...
        trades.append(
            {
                "entry_date": row["date"],
                "probability_outperform": float(row["probability_outperform"]),
                "expected_excess_log_return": float(row.get("expected_excess_log_return", np.nan)),
                "position": position,
                ...
                "turnover": turnover,
                "bullish_threshold": use_bullish,
                "bearish_threshold": use_bearish,
                "minimum_edge_bps": use_edge_bps,
            }
        )
    ...
    if trades_df.empty:
        return trades_df, pd.DataFrame([{"label": label, "n_trades": 0}])
    ...
    returns = trades_df["net_log_return"].to_numpy(dtype=float)
    sharpe = np.nan
        ...
        [
            {
                ...
                "avg_net_return_per_signal_pct": float((np.exp(np.mean(returns)) - 1.0) * 100.0),
                "turnover_avg": float(trades_df["turnover"].mean()),
                "transaction_cost_bps": float(cfg.transaction_cost_bps),
                "bullish_threshold": float(use_bullish),
                "bearish_threshold": float(use_bearish),
                "minimum_edge_bps": float(use_edge_bps),
...
def optimize_probability_thresholds(predictions: pd.DataFrame, cfg: Config, output_dir: Path) -> dict[str, float]:
    bullish_candidates = np.round(np.arange(0.50, 0.81, 0.05), 2)
    bearish_candidates = np.round(np.arange(0.20, 0.51, 0.05), 2)
    edge_candidates = sorted({0.0, cfg.transaction_cost_bps, cfg.transaction_cost_bps * 2.0, 10.0, 20.0, 40.0})
    rows = []
    best_row: dict[str, float] | None = None
    best_score = (-np.inf, -np.inf, -np.inf, -np.inf, -np.inf)

    for bullish_threshold in bullish_candidates:
        for bearish_threshold in bearish_candidates:
            if bearish_threshold >= bullish_threshold:
                continue
            for minimum_edge_bps in edge_candidates:
                _, metrics_df = run_backtest(
                    predictions,
                    cfg,
                    output_dir,
                    label="validation_optimized_candidate",
                    bullish_threshold=float(bullish_threshold),
                    bearish_threshold=float(bearish_threshold),
                    minimum_edge_bps=float(minimum_edge_bps),
                    write_outputs=False,
                )
                metrics = metrics_df.iloc[0].to_dict()
                trades_nonzero = float(metrics.get("n_trades_nonzero", 0.0) or 0.0)
                sharpe = float(metrics.get("annualized_sharpe", np.nan))
                total_return = float(metrics.get("total_return_pct", np.nan))
                max_drawdown = float(metrics.get("max_drawdown_pct", np.nan))
                hit_rate = float(metrics.get("hit_rate_nonzero", np.nan)) if trades_nonzero > 0 else -1.0
                turnover_penalty = -float(metrics.get("turnover_avg", np.nan)) if trades_nonzero > 0 else -np.inf
                score = (
                    sharpe if np.isfinite(sharpe) else -np.inf,
                    total_return if np.isfinite(total_return) else -np.inf,
                    hit_rate if np.isfinite(hit_rate) else -np.inf,
                    -abs(max_drawdown) if np.isfinite(max_drawdown) else -np.inf,
                    turnover_penalty,
                )
                row = {"bullish_threshold": bullish_threshold, "bearish_threshold": bearish_threshold, "minimum_edge_bps": minimum_edge_bps, **metrics}
                rows.append(row)
                if score > best_score:
                    best_score = score
                    best_row = row

    pd.DataFrame(rows).sort_values(by=["annualized_sharpe", "total_return_pct", "hit_rate_nonzero"], ascending=[False, False, False]).to_csv(
        output_dir / "validation_threshold_search.csv",
        index=False,
    )
    if best_row is None:
        return {"bullish_threshold": cfg.bullish_threshold, "bearish_threshold": cfg.bearish_threshold, "minimum_edge_bps": cfg.minimum_edge_bps}
    return {"bullish_threshold": float(best_row["bullish_threshold"]), "bearish_threshold": float(best_row["bearish_threshold"]), "minimum_edge_bps": float(best_row["minimum_edge_bps"])}


def run_simple_baseline_classification(partitions: dict[str, dict[str, np.ndarray]], output_dir: Path) -> pd.DataFrame:
    y_train = partitions["train"]["targets"].astype(int)
    ...
    for split in ["valid", "test"]:
        ...
        for name, prob in baselines.items():
            rows.append({"model": name, "split": split, "train_outperform_rate": train_base_rate, **classification_metrics(y, prob)})
    df = pd.DataFrame(rows)
    df.to_csv(output_dir / "simple_baseline_comparison.csv", index=False)
    return df


def run_fixed_position_backtest(predictions: pd.DataFrame, cfg: Config, output_dir: Path, label: str, position: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    df = predictions.dropna(subset=["future_stock_log_return", "future_benchmark_log_return"]).copy().sort_values("date").reset_index(drop=True)
    step = max(1, int(cfg.forecast_horizon_days))
    trades = []
    previous_position = 0
    for i in range(0, len(df), step):
        ...
        cost_log_return = turnover * 2.0 * cfg.transaction_cost_bps / 10000.0
        net_log_return = gross_log_return - cost_log_return
        trades.append({"entry_date": row["date"], "baseline": label, "position": int(position), "gross_log_return": gross_log_return, "cost_log_return": cost_log_return, "net_log_return": net_log_return, "net_return_pct": (np.exp(net_log_return) - 1.0) * 100.0, "stock_future_log_return": float(row["future_stock_log_return"]), "benchmark_future_log_return": float(row["future_benchmark_log_return"]), "spread_log_return": spread_log_return, "hit": bool(net_log_return > 0.0) if position != 0 else np.nan, "turnover": turnover})
    ...
    if np.std(returns) > 1e-12:
        sharpe = float(np.mean(returns) / np.std(returns) * np.sqrt(periods_per_year))
    nonzero = trades_df["position"] != 0
    metrics_df = pd.DataFrame([{"label": label, "n_signals": int(len(trades_df)), "n_trades_nonzero": int(nonzero.sum()), "fixed_position": int(position), "total_return_pct": float((equity[-1] - 1.0) * 100.0), "annualized_sharpe": sharpe, "max_drawdown_pct": float(max_drawdown * 100.0), "hit_rate_nonzero": float(trades_df.loc[nonzero, "hit"].mean()) if nonzero.any() else np.nan, "avg_net_return_per_signal_pct": float((np.exp(np.mean(returns)) - 1.0) * 100.0), "turnover_avg": float(trades_df["turnover"].mean()), "transaction_cost_bps": float(cfg.transaction_cost_bps)}])
    trades_df.to_csv(output_dir / f"backtest_{label}_trades.csv", index=False)
    metrics_df.to_csv(output_dir / f"backtest_{label}_metrics.csv", index=False)
    plot_backtest_equity(trades_df, cfg, output_dir / f"plot_backtest_{label}_equity.png")
    return trades_df, metrics_df
...
def run_simple_baseline_backtests(predictions: pd.DataFrame, cfg: Config, output_dir: Path, label_prefix: str = "test") -> pd.DataFrame:
    baseline_specs = [(f"{label_prefix}_simple_always_long_stock_short_benchmark", 1), (f"{label_prefix}_simple_always_neutral", 0)]
    if cfg.allow_short:
...
def plot_backtest_equity(trades_df: pd.DataFrame, cfg: Config, output_path: Path) -> None:
    ...
    ax1.set_title(f"Backtest equity: {cfg.ticker.upper()} vs {cfg.benchmark.upper()}, horizon={cfg.forecast_horizon_days}, cost={cfg.transaction_cost_bps:.1f} bps")
    fig.tight_layout()
