...
def run_once(*, args: argparse.Namespace, broker, config: LLMOptionsRuntimeConfig, provider: str) -> dict:
    ...
    if args.enable_profit_policy_llm:
        _, profit_policy_raw_response, profit_policy_decision = call_response(
            ...
            usage_context=_usage_context(args=args, config=config, provider=profit_policy_profile.provider, process="llm_options_profit_policy"),
        )
    ...
    if has_shadow_exposure:
        ...
        _, exit_raw_response, exit_decision = call_response(
            ...
            search_context_size=args.search_context_size,
            reasoning_effort=args.reasoning_effort,
            usage_context=_usage_context(args=args, config=config, provider=exit_profile.provider, process="llm_options_exit_agent"),
        )
        ...
        _, entry_raw_response, entry_decision = call_response(
            ...
            search_context_size=args.search_context_size,
            reasoning_effort=args.reasoning_effort,
            usage_context=_usage_context(args=args, config=config, provider=entry_profile.provider, process="llm_options_entry_agent"),
        )
