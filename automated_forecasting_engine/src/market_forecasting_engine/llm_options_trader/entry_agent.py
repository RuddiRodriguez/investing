...
from market_forecasting_engine.llm_options_trader.prompts import ENTRY_JSON_SCHEMA, ENTRY_SYSTEM_MESSAGE, ENTRY_USER_MESSAGE
...
def main() -> None:
    ...
    while True:
        ...
        report_path = output_dir / f"{config.currency.upper()}_llm_entry_agent_report.json"
        write_json(report_path, record)
        append_jsonl(output_dir / "logs" / f"{config.currency.upper()}_llm_entry_agent.jsonl", record)
...
def run_once(*, args: argparse.Namespace, broker, config: LLMOptionsRuntimeConfig, provider: str, client) -> dict:
    ...
    packet = build_market_packet(broker=broker, config=config, process="entry_agent", now=now)
    ...
    _, raw_response, decision = call_response(
        ...
        system_message=ENTRY_SYSTEM_MESSAGE,
        user_message=ENTRY_USER_MESSAGE,
        json_schema=ENTRY_JSON_SCHEMA,
        ...
        usage_context={"process": "llm_options_entry_agent", "currency": config.currency, "instrument_currency": config.instrument_currency, "provider": provider},
    )
    ...
    if action == "submit_order":
        order_result = execute_order_payload(
            ...
            label=f"llm-entry-{config.currency.upper()}-{now.strftime('%Y%m%d%H%M%S')}",
        )
    else:
    ...
    append_memory_event(Path(args.output_dir), config.currency, compact_decision_event(process="entry_agent", record=record))
    return record
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="LLM-authoritative Deribit testnet option entry agent.")
