"""Graph test for method 3: moving-average crossover with no-trade zone."""

import plotly.express as px

try:
    from scripts.direction.common import CSV_FILE, PRICE_COLUMN, add_prediction_correctness
    from scripts.direction.method_3_moving_average_crossover import add_signal, read_prices
except ImportError:
    from common import CSV_FILE, PRICE_COLUMN, add_prediction_correctness
    from method_3_moving_average_crossover import add_signal, read_prices


def add_predictions(prices):
    output = add_signal(prices)
    output = add_prediction_correctness(output)
    output = output.dropna(subset=["Short Average", "Long Average", "Actual"])
    return output
...
def plot_predictions(prices):
    ...
    fig = px.scatter(
        ...
        title="Method 3 Prediction Test",
    )
    fig.add_scatter(
        x=prices["Date"],
        y=prices["Short Average"],
        mode="lines",
        name="Short Average",
    )
    fig.add_scatter(
        x=prices["Date"],
        y=prices["Long Average"],
        mode="lines",
        name="Long Average",
    )
    fig.show()
