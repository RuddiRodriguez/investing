"""Graph test for method 4: weighted indicator voting with no-trade zone."""

import plotly.express as px

try:
    from scripts.direction.common import CSV_FILE, PRICE_COLUMN, add_prediction_correctness
    from scripts.direction.method_4_indicator_voting import add_signal, read_prices
except ImportError:
    from common import CSV_FILE, PRICE_COLUMN, add_prediction_correctness
    from method_4_indicator_voting import add_signal, read_prices


def add_predictions(prices):
    output = add_signal(prices)
    output = add_prediction_correctness(output)
    output = output.dropna(subset=["Vote Score", "Actual"])
    return output
...
def plot_predictions(prices):
    ...
    fig = px.scatter(
        ...
        title="Method 4 Prediction Test",
    )
    fig.add_scatter(
        x=prices["Date"],
